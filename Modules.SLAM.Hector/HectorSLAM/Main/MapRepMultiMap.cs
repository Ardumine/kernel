...
namespace HectorSLAM.Main
{
    ...
    public class MapRepMultiMap
    {
        ...
        /// Grid maps.
        /// </summary>
        public OccGridMap[] Maps { get; set; }
        ...
        public MapRepMultiMap(float mapResolution, Point mapSize, int numDepth, Vector2 startCoords)
        {
            ...
            {
                //Vars.Log($"HectorSLAM map level #{i} resolution: {mapResolution} m/pix, size: {mapSize}");
        ...
        public List<LogOddsCell[]> GetMaps()
        {
            var l = new List<LogOddsCell[]>();
        ...
        public void SetMaps(List<LogOddsCell[]> _maps)
        {
            for (int i = 0; i < Maps.Length; i++)
            {
                Maps[i].mapArray = _maps[i];
            }
        }
