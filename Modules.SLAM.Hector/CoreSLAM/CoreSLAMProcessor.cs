...
namespace CoreSLAM
{
    ...
    public class CoreSLAMProcessor : IDisposable
    {
        ...
        private static void ScanSegmentsToCloud(ScanSegment segment, Vector3 odometryPose, out ScanCloud cloud)
        {
            cloud = new ScanCloud();
                ...
                for (int i = 0; i < segment.Rays.Count; i++)
                {
                    Ray r = segment.Rays[i];
        ...
        private void DrawLaserRayOnHoleMap(int x1, int y1, int x2, int y2, int xp, int yp, int value, int alpha)
        {
            ...
            for (int x = 0; x <= dxc; x++, ptr += incptrx)
            {
                ...
                var dd = HoleMap.Pixels[ptr];
                HoleMap.Pixels[ptr] = (ushort)(((256 - alpha) * dd + alpha * pixval) >> 8);
        ...
        /// </summary>
        public void Update(ScanSegment segment)
        {
            Vector3 odoPose = segment.Pose;
            Vector3 newPose;

            // Convert scan segments to cloud
            ScanSegmentsToCloud(segment, odoPose, out ScanCloud cloud);
